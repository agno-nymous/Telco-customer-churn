...
if uploaded_file:
    test_df = pd.read_csv(
        uploaded_file
    )  # .sort_values(by=['customerID']) #reading the csv uploaded csv file
    response_enc = (
        resp_enc.response_encoding()
    )  # creating an object of the response encoding class
    response_enc = joblib.load(
        "./model/resp_enc.pkl"
    )  # loading the response encoding model
    ...
    y_pred_proba = load_model().predict_proba(x_test)[
        :, 1
    ]  # predicting the probabilities of the test dataframe
    y_pred = load_model().predict(
        x_test
    )  # predicting the response of the test dataframe
